...
- **Tích hợp Notification Khẩn cấp qua Telegram:** Mọi biến động đơn đặt lịch (có booking mới, đổi lịch, v.v.) đều được đẩy thẳng vào điện thoại quản lý thông qua **Telegram Bot**. Người điều hành không cần túc trực máy tính mà vẫn chốt lịch nhanh chóng.
- **Tự động hóa luồng Email thông minh:** Hệ thống hỗ trợ Queue gửi email bất đồng bộ qua **Gmail Service**. Giao diện template email được thiết kế HTML chỉn chu, gửi xác nhận lịch hẹn chính xác lập tức mà không gây độ trễ (lag) cho thao tác tải trang của người dùng.
- **Tác vụ ngầm (Background Jobs & Schedulers):** Xây dựng riêng các công nhân chạy nền (Queue Workers). Tiêu biểu như tính năng tự động quét và đánh dấu Hết hạn đơn chưa thanh toán sau 24h, tự động đối soát giao dịch định kỳ, giảm thiểu mọi nghiệp vụ rà soát thủ công của con người.
- **API chuyên biệt và Webhook Thanh toán:** Tích hợp thanh toán QR Code (ngoài hệ thống hoặc thông qua trung gian như SePay), với API chuyên dụng tự động kiểm duyệt phiên giao dịch, không còn xác nhận chuyển khoản bằng mắt thường.
- **Bảo mật giới hạn truy cập (Rate Limit & Throttle):** Trang bị logic Middleware chống spam trên mọi đầu API mở (như gửi OTP, đặt hẹn), bảo vệ hệ thống khỏi các đợt ráo riết tấn công DDoS cơ bản.
